...
export default function LiveChatSniper({ onClose, globalBoggsOverride }: LiveChatSniperProps) {
  const [panelists, setPanelists] = useState<{ id: string, name: string, alias: string }[]>([
    ...
    { id: 'jc2pointzero',      name: 'jc2pointzero',      alias: 'James'         },
  ]);
  ...
  const PERSONA_VOICES: Record<string, string> = {
    ...
    jc2pointzero: "You are jc2pointzero, an always hopeful, optimistic Mets fan. You love this team and believe in 2026 magic (a retool, not a rebuild). You absolutely hate Bo Bichette though. Keep it under 200 characters.",
  };
       ...
       {shotModal && (
           ...
           >
               ...
               >
                   ...
                   {shotModal.state === 'drafting' ? (
                       <div className="flex flex-col gap-4">
                           <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-3 text-sm">
                               <div className="text-blue-400 font-bold mb-1">Targeting {shotModal.targetAuthor}:</div>
                               <div className="text-white/80 italic">"{shotModal.targetText}"</div>
                           </div>
                           <div>
                               <div className="text-[10px] text-white/50 font-bold uppercase tracking-widest mb-2">Producer Notes / Context (Optional)</div>
                               <textarea
                                   value={draftNote}
                                   onChange={(e) => setDraftNote(e.target.value)}
                                   placeholder="e.g. This guy is a broken record, tell him to shut up..."
                                   className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-white text-sm resize-none outline-none focus:border-green-500/50"
                                   rows={3}
                                   autoFocus
                               />
                           </div>
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2 items-center border-l border-white/20 pl-4 ml-2">
                          <input 
                              ...
                              onKeyDown={(e) => {
                                  e.stopPropagation();
                                  if (e.key === 'Enter') handleStartSniping();
                              }}
                          />
                          <button 
                                           ...
                                           <button 
                                               ...
                                               onClick={() => {
                                                    ...
                                                    setShotModal({ 
                                                        ...
                                                        state: 'drafting', 
                                                        text: '',
                                                        targetAuthor: replyTarget.author,
                                                        targetText: replyTarget.text,
                                                        voice: voice
                                                    });
                                                    setReplyTarget(null);
                                                }}
