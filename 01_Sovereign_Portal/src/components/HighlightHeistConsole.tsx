...
import { Download, Link as LinkIcon, CheckCircle2, AlertCircle, RefreshCcw, FileText, Trash2, Eye, X, Copy, Play, Zap, EyeOff, Briefcase, Scissors } from 'lucide-react';

export default function HighlightHeistConsole() {
  ...
  const [analyzeStatus, setAnalyzeStatus] = useState<'idle' | 'analyzing' | 'complete' | 'error'>('idle');
  const [analyzeJobId, setAnalyzeJobId] = useState<string | null>(null);
  const [analyzeResult, setAnalyzeResult] = useState<{file?: string, filename?: string, error?: string} | null>(null);
  ...
  useEffect(() => {
      let interval: NodeJS.Timeout;
      if (analyzeStatus === 'analyzing' && analyzeJobId) {
          interval = setInterval(async () => {
              try {
                  const res = await fetch(`/api/snipe/${analyzeJobId}`);
                  const data = await res.json();
                  if (data.status === 'complete') {
  ...
  const handleAnalyze = async (filename?: string) => {
      const targetFile = filename || result?.file;
  ...
  return (
    <div className="h-[85vh] w-full flex flex-col p-6 bg-[#0B0E14] text-[#c5c6c7] font-['Segoe_UI',sans-serif] overflow-y-auto">
      ...
      <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
        <div className="w-10 h-10 rounded bg-[#a855f7]/20 flex items-center justify-center border border-[#a855f7]/50 ">
        ...
        <div>
          ...
          <p className="text-[12px] text-[#8E9CAA] uppercase tracking-widest font-mono">
            Covert Asset Extraction & Social Robbery Pipeline
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="bg-[#111827] border border-white/10 rounded-xl p-5 shadow-lg flex flex-col gap-4">
                <h3 className="text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2 mb-2">
                    <LinkIcon className="w-4 h-4 text-[#a855f7]" /> The Mark
                </h3>
                
                <div className="flex flex-col gap-2">
                    <label className="text-[10px] text-[#8E9CAA] uppercase tracking-widest font-bold">Content URL (X/Twitter, Reddit, etc)</label>
                    <input 
                        type="text" 
                        value={url}
                        onChange={(e) => setUrl(e.target.value)}
                        placeholder="https://x.com/..."
                        className="bg-black/50 border border-white/10 rounded p-3 text-white font-mono text-xs focus:border-[#a855f7]  outline-none transition-all"
                    />
                </div>
                ...
                <label className="flex items-center gap-2 cursor-pointer mt-2 text-[#8E9CAA] font-mono text-[10px] uppercase tracking-widest hover:text-white transition-colors w-fit">
                    <input 
                        ...
                        className="w-4 h-4 rounded border-white/20 bg-black/50 text-[#a855f7] focus:ring-[#a855f7]"
                    />
                    Extract Evidence (Comments)
                </label>

                <button 
                    onClick={handleHeist}
                    disabled={status === 'downloading' || !url}
                    className="w-full mt-4 flex items-center justify-center gap-2 py-3 px-6 bg-[#a855f7] hover:bg-[#9333ea] disabled:bg-[#a855f7]/50 disabled:cursor-not-allowed text-white font-bold text-sm uppercase tracking-widest rounded-md  transition-all border-none"
                >
                    {status === 'downloading' ? <RefreshCcw className="w-4 h-4 animate-spin" /> : <Briefcase className="w-4 h-4" />}
                    {status === 'downloading' ? 'Executing Heist...' : 'Execute Heist'}
                </button>
            </div>
            ...
            <div className="bg-[#111827] border border-white/10 rounded-xl p-5 shadow-lg flex-1 flex flex-col">
                ...
                <div className="flex-1 bg-black/50 border border-white/5 rounded p-4 font-mono text-[10px] text-[#38bdf8] overflow-y-auto space-y-2">
                    <p className="text-gray-500">&gt; OPERATION HIGHLIGHT HEIST INITIALIZED.</p>
                    <p className="text-gray-500">&gt; AWAITING THE MARK.</p>
                    
                    {status === 'downloading' && (
                        <>
                            <p className="text-[#a855f7]">&gt; TARGET ACQUIRED: {url}</p>
                            <p className="text-[#a855f7] animate-pulse">&gt; BYPASSING SECURITY WITH yt-dlp INGESTION PIPELINE...</p>
                        </>
                    )}
                    ...
                    {status === 'complete' && result && (
                        <>
                            <p className="text-[#a855f7]">&gt; TARGET ACQUIRED: {url}</p>
                            <p className="text-[#a855f7]">&gt; PIPELINE COMPLETE.</p>
                            <p className="text-[#00ff00]">&gt; ASSET STOLEN: {result.filename}</p>
                            <p className="text-[#00ff00]">&gt; SECURED IN: /media_vault/01_Ingest/</p>
                        </>
                    )}
                    
                    {status === 'error' && result && (
                        <>
                            <p className="text-[#ff0033]">&gt; HEIST FAILED. COPS ARE ON THE WAY.</p>
        ...
        <div className="lg:col-span-2 bg-[#111827] border border-white/10 rounded-xl shadow-lg flex flex-col overflow-hidden">
            <div className="px-5 py-3 border-b border-white/10 flex justify-between items-center bg-[#0B0E14]">
                ...
                {status === 'complete' && (
                    <span className="flex items-center gap-1 text-[10px] text-[#00ff00] font-bold tracking-widest uppercase bg-[#00ff00]/10 px-2 py-1 rounded">
                        <CheckCircle2 className="w-3 h-3" /> STOLEN
                    </span>
                )}
            </div>
            <div className="flex-1 bg-black relative flex items-center justify-center p-4">
                ...
                {status === 'downloading' && (
                    <div className="w-full flex flex-col items-center justify-center gap-6 p-8 bg-black/40 rounded-xl border border-[#a855f7]/30 shadow-inner">
                        <div className="flex flex-col items-center gap-3 text-[#a855f7] font-bold text-sm uppercase tracking-widest text-center">
                            <RefreshCcw className="w-12 h-12 animate-spin mb-2" />
                            <p>Smuggling Web Asset...</p>
                        ...
                        <div className="w-full max-w-md bg-white/5 rounded-full h-4 border border-white/10 overflow-hidden relative shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)]">
                            <div 
                                className="h-full bg-gradient-to-r from-[#a855f7]/50 to-[#a855f7]  rounded-full transition-all duration-300" 
                                style={{ width: `${result?.progress || 0}%` }}
                            ></div>
                        </div>
                        
                        <div className="text-[#a855f7] font-mono text-2xl font-black tracking-widest drop-">
                    ...
                    <div className="w-full h-full border border-[#00ff00]/30 rounded-lg overflow-hidden relative  flex items-center justify-center bg-[#0B0E14] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#00ff00]/10 via-[#0B0E14] to-[#0B0E14]">
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
                             <div className="bg-black/60 border border-[#00ff00]/50 p-8 w-full max-w-md rounded-2xl backdrop-blur-xl shadow-2xl flex flex-col items-center">
                                 ...
                                 <h2 className="text-white font-black text-2xl uppercase tracking-[0.2em] mb-2 drop-">Asset Stolen</h2>
                                 <p className="text-[#a855f7] font-mono text-xs mb-8 break-all max-w-xs">{result?.filename}</p>
                                 ...
                                 {transcribeStatus === 'transcribing' && (
                                    <div className="w-full flex flex-col items-center gap-3 mt-4 bg-black/40 p-4 rounded-xl border border-[#a855f7]/30">
                                        <div className="flex items-center gap-2 text-[#a855f7] font-bold text-[10px] uppercase tracking-widest">
                                        ...
                                        <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden shadow-inner">
                                            <div className="h-full bg-[#a855f7] w-1/2 animate-[pulse_1s_ease-in-out_infinite] rounded-full"></div>
                                        </div>
                                    </div>
                                 )}
                                 ...
                                 {transcribeStatus === 'complete' && transcriptResult && (
                                    <a 
                                        ...
                                        className="w-full flex items-center justify-center gap-3 py-4 px-6 bg-[#a855f7]/20 hover:bg-[#a855f7]/40 border border-[#a855f7]/50 text-[#a855f7] font-bold text-sm uppercase tracking-widest rounded-xl transition-all  hover:scale-105"
                                    >
                                 ...
                                 {analyzeStatus === 'idle' && (
                                    <button 
                                        ...
                                        className="w-full mt-3 flex items-center justify-center gap-3 py-4 px-6 bg-[#38bdf8]/20 hover:bg-[#38bdf8]/40 border border-[#38bdf8]/50 text-[#38bdf8] font-bold text-sm uppercase tracking-widest rounded-xl transition-all  hover:scale-105"
                                    >
                                        <Zap className="w-5 h-5" /> Analyze (Local AI)
                                    </button>
                                 )}

                                 {analyzeStatus === 'analyzing' && (
                                    <div className="w-full flex flex-col items-center gap-3 mt-4 bg-black/40 p-4 rounded-xl border border-[#38bdf8]/30">
                                        <div className="flex items-center gap-2 text-[#38bdf8] font-bold text-[10px] uppercase tracking-widest">
                                            <RefreshCcw className="w-4 h-4 animate-spin" /> LLaVA/Llama3 Processing...
                                        </div>
                                        <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden shadow-inner">
                                            <div className="h-full bg-[#38bdf8] w-1/2 animate-[pulse_1s_ease-in-out_infinite] rounded-full"></div>
                                        </div>
                                    </div>
                                 )}
        ...
        <div className="lg:col-span-1 bg-[#111827] border border-white/10 rounded-xl p-5 shadow-lg flex flex-col min-h-0">
            <h3 className="text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2 mb-4">
                <Briefcase className="w-4 h-4 text-[#00ff00]" /> Loot Vault
            </h3>
            <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                {history.length === 0 ? (
                    <div className="text-[#8E9CAA] text-xs font-mono uppercase text-center mt-10">Vault is empty</div>
                ) : (
                    history.map((file, idx) => (
                        <div key={idx} className="bg-black/50 border border-white/5 p-3 rounded-lg flex flex-col gap-2 hover:border-[#a855f7]/50 transition-colors group">
                            <div className="flex justify-between items-start gap-2">
                                <span className="text-[#a855f7] font-mono text-xs break-words line-clamp-2 group-hover:text-white transition-colors flex-1">{file.filename}</span>
                                <div className="flex items-center gap-1">
                                    {(file.filename.endsWith('.md') || file.filename.endsWith('.json')) && (
                                        <button 
                                            onClick={() => handleViewTranscript(file.filename)}
                                            className="text-[#a855f7]/50 hover:text-[#a855f7] transition-colors p-1"
                                            title="View File"
                                        >
                                    ...
                                    {file.filename.endsWith('_transcript.md') && (() => {
                                        ...
                                        return (
                                            <button 
                                                ...
                                                className="text-[#a855f7]/50 hover:text-[#a855f7] transition-colors p-1"
                                                title={isSummarizing ? "Summarizing..." : "Generate TL;DR (Llama 3)"}
                                            >
                                    ...
                                    {file.filename.endsWith('.mp4') && (() => {
                                        ...
                                        const isAnalyzing = Object.values(activeJobs).some(j => j.status === 'analyzing' && j.filepath?.endsWith(file.filename));
                                        return (
                                            <>
                                                <button 
                                                ...
                                                <button 
                                                    onClick={() => !isAnalyzing && handleAnalyze(file.filename)}
                                                    className="text-[#38bdf8]/50 hover:text-[#38bdf8] transition-colors p-1"
                                                    title={isAnalyzing ? "Analyzing..." : "Analyze Video (Local AI)"}
                                                >
                                                    {isAnalyzing ? <RefreshCcw className="w-3 h-3 animate-spin" /> : <Zap className="w-3 h-3" />}
                                                </button>
                                            </>
                                        );
                                    })()}
      ...
      {viewFileName && viewFileContent !== null && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
              <div className="bg-[#111827] border border-[#a855f7]/30 rounded-xl w-full max-w-4xl max-h-[90vh] flex flex-col ">
                  <div className="flex justify-between items-center p-4 border-b border-white/10 shrink-0">
                      <h3 className="text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2">
                          <Eye className="w-4 h-4 text-[#a855f7]" /> {viewFileName}
                      </h3>
                  ...
                  <div className="p-4 border-t border-white/10 flex justify-end shrink-0">
                      <button onClick={handleCopy} className="flex items-center gap-2 py-2 px-4 bg-[#a855f7]/20 hover:bg-[#a855f7]/40 border border-[#a855f7]/50 text-[#a855f7] font-bold text-sm uppercase tracking-widest rounded transition-all">
      ...
      {viewMediaFilename && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 md:p-6">
              <div className="bg-[#111827] border border-[#ff0033]/30 rounded-xl w-full max-w-5xl max-h-[90vh] flex flex-col  overflow-hidden">
                  <div className="flex justify-between items-center p-4 border-b border-white/10 shrink-0 bg-black/50">
                      <h3 className="text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2 truncate">
                          <Play className="w-4 h-4 text-[#ff0033] shrink-0" /> {viewMediaFilename}
                      </h3>
                      <button onClick={() => setViewMediaFilename(null)} className="text-white/50 hover:text-white p-1 shrink-0 ml-4">
                          <X className="w-5 h-5" />
                      </button>
                  </div>
                  <div className="w-full bg-black flex items-center justify-center relative flex-1 min-h-0">
                      <video 
                          src={`/api/snipe/media/${encodeURIComponent(viewMediaFilename)}`} 
                          controls 
                          autoPlay 
                          className="w-full h-full max-h-[calc(90vh-70px)] object-contain outline-none"
                      />
                  </div>
              </div>
          </div>
