...
export default function MobileHololink() {
  ...
  const myRoleRef = useRef<string>(''); // tracks own role to prevent self-ring

  const authUser = useAuth();
  const defaultId = useRef('mobile_' + Math.random().toString(36).substring(7));
  const myId = authUser?.user_name || defaultId.current;
  const myDisplayName = authUser?.display_name || 'Mobile Guest';
  const myRole = authUser?.role || 'patron';
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        if (msg.type === 'WEBRTC_OFFER') {
          ...
          if (target === targetRole || target === 'mobile_hololink' || target === myId) {
            ...
            window.sessionStorage.setItem('target_role', target);
          }
        }

        if (msg.type === 'WEBRTC_ICE_CANDIDATE') {
          const targetRole = branding === 'aether_vet' ? 'aether_vet_hq' : 'clio';
          const myRoleVal = window.sessionStorage.getItem('target_role') || myId;
          const target = msg.target || msg.to;
          if (target === targetRole || target === 'mobile_hololink' || target === myRoleVal || target === myId) {
        ...
        if (msg.type === 'WEBRTC_ANSWER') {
          const myRoleVal = window.sessionStorage.getItem('target_role') || myId;
          const target = msg.target || msg.to;
          if (target === myRoleVal || target === 'mobile_hololink' || target === myId) {
            if (peerConnectionRef.current) {
  ...
  const makeCall = async () => {
    ...
    try {
      ...
      const myRoleVal = myId;
      myRoleRef.current = myRoleVal; // store so onmessage can filter out self-reflected broadcasts
      const targetRole = branding === 'aether_vet' ? 'aether_vet_hq' : (branding === 'fanstack' ? 'fanstack_hq' : 'clio');
      
      window.sessionStorage.setItem('target_role', myRoleVal);
