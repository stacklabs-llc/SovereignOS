  ...
  const newsDeskColumns = [
    {
      ...
      render: (anom: any) => (
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-8 h-8 bg-[#38bdf8]/10 rounded flex items-center justify-center text-[#38bdf8]">
            <AlertCircle className="w-4 h-4" />
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-white uppercase tracking-wider text-xs">{anom.event}</span>
            <span className="text-[10px] font-mono text-white/40">{anom.time}</span>
          </div>
        </div>
      )
    },
    {
      ...
      sortable: true,
      render: (anom: any) => (
        <span className="bg-white/5 px-2 py-0.5 rounded text-white/80">{anom.persona}</span>
      )
    },
    {
      key: 'format',
      ...
      sortable: true,
      render: (anom: any) => (
        <span className="text-[#ef4444] border border-[#ef4444]/20 bg-[#ef4444]/10 px-2 py-0.5 rounded text-[9px] uppercase font-bold">
          {anom.format}
        </span>
      )
    },
    {
      key: 'script',
      label: 'Advocate Script',
      sortable: true,
      render: (anom: any) => (
        <span className="italic text-white/60 text-xs">"{anom.script}"</span>
      )
    },
    {
      key: 'actions',
      ...
      render: (anom: any) => (
        <div className="flex gap-2">
          <button 
             ...
             className="bg-[#a855f7]/20 hover:bg-[#a855f7]/40 border border-[#a855f7]/50 text-[#a855f7] px-2.5 py-1 rounded text-[9px] tracking-widest uppercase font-bold flex items-center justify-center gap-1 transition-all"
          >
             {copiedId === anom.id ? <CheckCircle className="w-3 h-3" /> : <Film className="w-3 h-3" />}
          </button>
          <button 
             onClick={() => handleReviewLater(anom.id)}
             className="bg-white/5 hover:bg-white/10 border border-white/10 text-white/50 hover:text-white px-2.5 py-1 rounded flex items-center justify-center transition-all"
          >
    ...
    <div className="w-full max-w-[1400px] mx-auto p-4 flex flex-col gap-4 h-full">
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0">
        {viewMode === 'grid' ? (
          ...
          <SortableTable
            data={anomalies}
            columns={newsDeskColumns}
            searchPlaceholder="Search anomalies..."
            searchKeys={["event", "persona", "format", "script"]}
            theme="glass"
          />
        )}
      </div>
