...
export default function PromoInbox() {
  ...
  const loadEmails = () => {
    fetch('/promo_staging.json')
      .then(res => res.json())
      .then(data => setEmails(data))
      .catch(err => {
         console.warn("promo_staging.json not found, loading fallback empty inbox", err);
         setEmails([]);
      })
  ...
  const handleFetchMailbag = async () => {
    ...
    try {
      ...
      setFeedback(`Successfully swept ${data.staged_count || 0} emails!`);
      loadEmails();
    } catch (err) {
      console.error(err);
  ...
  const handleAction = (id: string, action: 'keep' | 'trash') => {
    setEmails(prev => prev.filter(e => e.id !== id));
    console.log(`Email ${id} action: ${action}`);
  };
