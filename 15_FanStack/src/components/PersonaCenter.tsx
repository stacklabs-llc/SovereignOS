...
export default function PersonaCenter() {
  ...
  const handleExportMD = () => {
    ...
    let mdStr = `# Sovereign OS — AI Personas Export\n\n`;
    ...
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `sovereign_personas_export.md`);
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
  };
                ...
                <div style={{ flex: 1, padding: "18px", display: "flex", flexDirection: "column", position: "relative", zIndex: 4, background: "#0d1820" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "16px", background: "rgba(0,0,0,0.2)", padding: "10px", borderRadius: "6px", border: `1px solid ${VM.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontFamily: VM.fontMono, fontSize: "0.6rem", color: VM.muted, textTransform: "uppercase" }}>Engine</span>
                        <span style={{ fontFamily: VM.fontMono, fontSize: "0.68rem", color: VM.text, fontWeight: "bold" }}>{r.u_llm_engine || "gemini-2.5-flash"}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {activeRecords.map((record) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Engine cell — double-click to inline edit */}
                    <td
                      onDoubleClick={(e) => startInlineEdit(e, record.sys_id, "u_llm_engine", bot.u_llm_engine ?? "gemini-2.5-flash")}
                      title="Double-click to edit engine"
                      style={{ padding: "8px 14px", fontFamily: VM.fontMono, fontSize: "0.75rem", color: VM.text, cursor: "cell", minWidth: "120px" }}>
                      {inlineEdit?.sys_id === record.sys_id && inlineEdit.field === "u_llm_engine" ? (
                        <input autoFocus value={inlineEdit.value}
                          onChange={(e) => setInlineEdit({ ...inlineEdit, value: e.target.value })}
                          onBlur={commitInlineEdit}
                          onKeyDown={(e) => { if (e.key === "Enter") commitInlineEdit(); if (e.key === "Escape") setInlineEdit(null); }}
                          onClick={(e) => e.stopPropagation()}
                          style={{ background: VM.surface, border: `1px solid ${VM.blue}`, borderRadius: "4px", color: VM.text, fontFamily: VM.fontMono, fontSize: "0.75rem", padding: "2px 4px", outline: "none", width: "100%", boxSizing: "border-box" }} />
                      ) : (
                        <span style={{ borderBottom: `1px dashed ${VM.border}`, paddingBottom: "1px" }}>
                          {bot.u_llm_engine || "gemini-2.5-flash"}
                        </span>
                      )}
                    </td>
                  ...
                  {[
                    ...
                    <div key={key} style={{ gridColumn: type === "entropy-slider" ? "span 2" : "span 1" }}>
                      ...
                      {type === "team-select" ? (
                        ...
                        <select value={String(editForm[key] || "style_felt")} onChange={(e) => setEditForm({ ...editForm, [key]: e.target.value })}
                          style={{ width: "100%", background: VM.surface, border: `1px solid ${VM.border}`, borderRadius: "6px", color: VM.text, padding: "10px 12px", fontFamily: VM.fontMono, fontSize: "0.82rem", outline: "none" }}
                          onFocus={(e) => e.target.style.borderColor = VM.emerald} onBlur={(e) => e.target.style.borderColor = VM.border}>
          ...
          <div style={{
              ...
              <div style={{
                  ...
                  <p style={{ fontFamily: VM.fontMono, fontSize: "0.8rem", color: VM.text, marginBottom: "16px" }}>
                      Updating {selectedRecords.size} {activeTab === "ai_bots" ? "Personas" : "Users"}. Fields left blank will not be modified.
                  </p>
