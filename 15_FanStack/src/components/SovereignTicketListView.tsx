...
export default function SovereignTicketListView({ tasks, onEditTask, onRefresh }: SovereignTicketListViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('All');
  const [priorityFilter, setPriorityFilter] = useState<string>('All');
  const [sortField, setSortField] = useState<keyof AgentTask>('id');
  ...
  const filteredTasks = tasks.filter(t => {
    if (searchTerm && !t.title.toLowerCase().includes(searchTerm.toLowerCase()) && !t.id.toLowerCase().includes(searchTerm.toLowerCase())) {
      return false;
    }
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full gap-4 text-white font-['Rajdhani',sans-serif]">
      ...
      {/* Sidebar Filter */}
      <div className="w-full lg:w-56 shrink-0 bg-[#0a1118]/80 border border-white/10 rounded-xl p-4 flex flex-col gap-4 backdrop-blur-md">
        <div className="flex items-center gap-2 text-[#38bdf8] font-bold uppercase tracking-widest border-b border-white/10 pb-3">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0a1118]/80 border border-white/10 rounded-xl backdrop-blur-md overflow-hidden">
        ...
        {/* Toolbar */}
        <div className="p-4 border-b border-white/10 flex items-center justify-between bg-black/40 min-h-[65px] gap-4 flex-wrap lg:flex-nowrap">
          {selectedIds.length > 0 ? (
            ...
            <>
              ...
              <div className="text-xs font-mono text-[#8E9CAA] uppercase tracking-widest flex items-center gap-2">
                 ...
                 Showing {sortedTasks.length} of {tasks.length} tickets
              </div>
            </>
          )}
        </div>
        ...
        <div className="flex-1 overflow-x-auto lg:overflow-visible">
          <div className="min-w-0 lg:min-w-[800px]">
            {/* Table Header */}
            <div className="hidden lg:grid grid-cols-[40px_120px_1fr_100px_80px_140px_110px_110px_80px] gap-3 px-4 py-2 border-b border-white/10 bg-black/60 text-xs font-mono text-[#8E9CAA] uppercase tracking-widest font-bold select-none">
                ...
                <div className="divide-y divide-white/5">
                  {sortedTasks.map(task => (
                    <React.Fragment key={task.id}>
                      {/* Desktop View */}
                      <div className="hidden lg:grid grid-cols-[40px_120px_1fr_100px_80px_140px_110px_110px_80px] gap-3 px-4 py-3 items-center hover:bg-white/5 transition-colors group">
                        <div className="flex items-center justify-center">
                          <input 
                            type="checkbox" 
