...
*   **KI-061: Absolute Workstation Protection & Local Browser Spawn Ban (Fishbowl Safeguard)**  
    AI agents are STRICTLY AND PERMANENTLY FORBIDDEN from spawning local automated browsers, Playwright scripts, headful Chromium instances, or GUI testing windows of any kind natively on the Pilot's workstation (`clio`). Local browser spawning is a high-risk action that triggers a physical mechanical arm to knock a fishbowl with a living fish off the counter, resulting in severe physical damage and loss of life. All GUI, browser-level, or UAT validation MUST be delegated to external isolated Tailscale sandbox nodes (such as `metsy-prime` or `argo`) or verified via text-based API queries (`curl`). This invariant takes absolute priority over any other testing instructions.
...
graph TD
    ...
    HoloLink --> CardTurpeyUI[Card Turpey: 3016]
    HoloLink --> GardenStackUI[GardenStack: 3017]
    HoloLink --> CaryGrantUI[Cary Grant: 3018]
    HoloLink --> SpiteSliceUI[Spite Slice: 3019]
...
5. **Decoupled Apps:** HoloLink/Telepresence signaling (`8012`) -> AetherVet/Bistro (`3015`) -> Card Turpey (`3016`) -> GardenStack (`3017`) -> Cary Grant (`3018`) -> Spite Slice (`3019`)
...
6. **Frontend Dependency Audits:**
   ...
   - For each decoupled micro-frontend (Portal, Cinema, SDLC, Bistro, GardenStack, Card Turpey, Cary Grant, Spite Slice), run `npm install` inside their respective directories.
...
7. **System Core Service Startup:**
   ...
   - Run frontend dev builds on their mapped ports (3000, 3008, 3009, 3015, 3016, 3017, 3018, 3019).
